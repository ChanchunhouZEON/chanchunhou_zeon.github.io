<script lang="ts">
	import { projects } from '$lib/data/projects';
	import { reveal, spotlight } from '$lib/utils/reveal';
	import PageHeader from '$lib/components/PageHeader.svelte';
</script>

<svelte:head>
	<title>Projects - Open Source & Side Projects</title>
</svelte:head>

<div class="mx-auto max-w-6xl px-6 pt-32 pb-20">
	<PageHeader label="Projects" subtitle="Things I've built, from open source tools to full-stack applications.">
		All <span class="gradient-text">projects</span>
	</PageHeader>

	<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
		{#each projects as project, i}
			<div use:reveal={{ delay: i * 80, direction: 'tilt' }} use:spotlight class="card-3d glass group flex flex-col rounded-2xl p-6">
				<div class="relative z-10 mb-4 flex items-center justify-between">
					<div class="flex h-12 w-12 items-center justify-center rounded-xl bg-accent/8 text-accent dark:bg-accent/10 dark:text-accent-cyan">
						<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7 8-4 4 4 4"/><path d="m17 8 4 4-4 4"/><path d="m14 4-4 16"/></svg>
					</div>
					{#if project.featured}
						<span class="rounded-full bg-accent-yellow/10 px-2.5 py-0.5 text-xs font-semibold text-accent-yellow">Featured</span>
					{/if}
				</div>
				<h2 class="relative z-10 mb-2 text-lg font-bold transition-colors group-hover:text-accent dark:group-hover:text-accent-cyan">{project.title}</h2>
				<p class="relative z-10 mb-4 flex-1 text-sm leading-relaxed text-slate-400">{project.description}</p>
				<div class="relative z-10 mb-4 flex flex-wrap gap-2">
					{#each project.tech as t}
						<span class="tag-muted">{t}</span>
					{/each}
				</div>
				<div class="relative z-10 flex gap-4 border-t border-slate-200/40 pt-4 dark:border-slate-700/30">
					{#if project.github}
						<a href={project.github} target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-sm text-slate-400 transition-colors hover:text-accent dark:hover:text-accent-cyan">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
							Source
						</a>
					{/if}
					{#if project.demo}
						<a href={project.demo} target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-sm text-slate-400 transition-colors hover:text-accent dark:hover:text-accent-cyan">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
							Demo
						</a>
					{/if}
				</div>
			</div>
		{/each}
	</div>
</div>
